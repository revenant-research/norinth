...
| Prompt / completion **text** | SDK | **High** (may contain PHI/PII) | **Not sent by default.** The SDK sends a keyed HMAC fingerprint (`privacy.py`) — keyed by `NORINTH_SIGNING_SECRET` when set, otherwise by a key derived from the api key, so a default install never emits a bare, brute-forceable digest. Raw text only if `NORINTH_CAPTURE_CONTENT=1` is set on the application side. |
...
| Policy-declared intake fields | People via the intake form | Medium (business-confidential; **must not carry PHI**) | Only fields the governance policy declares are accepted: typed, length-capped (4,000 chars max), unknown keys rejected (`validate_custom_field_values`). Like the incident title, they label records across the product, so treat them as labels, never as clinical or personal narrative. |
| Governance policy documents | Org admins via the policy builder | Medium (business-confidential) | Versioned rows with a SHA-256 body hash; every activation is a hash-chained audit entry (`policy.activate`). Configuration, not code: no expressions, no templates. |
...
| Stolen or reset password on a local account | Per-user TOTP MFA (`services/totp.py`): the password step issues a short-lived challenge, never a session; codes are single-use (counter tracking) and secrets encrypted at rest. Recovery codes are hashed and single-use. An org admin can reset a member's MFA; the platform operator deliberately cannot — an operator password reset plus an operator MFA reset would add up to silent account takeover. |
