...
def test_setup_state_and_organization_flow(client, super_admin_client):
    ...
    # Setup is complete: state flips, and the wizard endpoint refuses a second run.
    ...
    # The operator signs in as the org admin with the password they chose — no forced rotation.
...
def test_setup_requires_admin_password_rotation_first(client):
    ...
    assert resp.status_code == 403  # blocked by the password-change middleware
