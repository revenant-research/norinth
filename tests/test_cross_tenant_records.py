"""one tenant's ingest key can't overwrite or re-tenant another tenant's records"""
...
def test_cross_tenant_record_overwrite_is_impossible(super_admin_client):
    ...
    # same human ids in both tenants across deployment, prompt, incident, eval
    ...
    # acme's passing eval still passing and attributed to acme
    evals = [e for e in acme.get("/api/evals").json()["evals"]]
    assert evals and all(e["passed"] for e in evals)

    # beta sees only its own values in its own tenant
    ...
    # one eval row per tenant, no cross-tenant dedup collision on t_ev/s_ev
